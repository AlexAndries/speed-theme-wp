...
class ST_CustomPostType {
  ...
  /**
   * Existing taxonomies to be registered after the posty has been registered
   *
  ...
  function __construct($post_type_names, $options = array()) {
    ...
    $this->add_action('init', array(
      &$this,
      'register_existing_taxonomies'
    ));
    // Add taxonomy to admin edit columns.
    $this->add_filter('manage_edit-' . $this->post_type_name . '_columns', array(
      &$this,
      'add_admin_columns'
    ));
    // Populate the taxonomy columns with the posts terms.
    $this->add_action('manage_' . $this->post_type_name . '_posts_custom_column', array(
      &$this,
      'populate_admin_columns'
    ), 10, 2);
    // Add filter select option to admin edit.
    $this->add_action('restrict_manage_posts', array(
      &$this,
      'add_taxonomy_filters'
    ));
    // rewrite post update messages
    $this->add_filter('post_updated_messages', array(
      &$this,
      'updated_messages'
    ));
    $this->add_filter('bulk_post_updated_messages', array(
      &$this,
   ...
   * @return $this
   */
  public function register_taxonomy($taxonomy_names, $options = array(), $hierarchical = true) {
   ...
   *
   * @return \SpeedTheme\Core\ST_CustomPostType
   */
      ...
      case (taxonomy_exists($column)) :
        ...
        if (!empty($terms)) {
          ...
          foreach ($terms as $term) {
            ...
            $output[] = sprintf(// Define link.
              '<a href="%s">%s</a>', // Create filter url.
              esc_url(add_query_arg(array(
                'post_type' => $post->post_type,
                $column     => $term->slug
              ), 'edit.php')), // Create friendly term name.
      ...
      default :
        ...
        if (isset($this->custom_populate_columns) && is_array($this->custom_populate_columns)) {
          ...
          if (isset($this->custom_populate_columns[$column]) && is_callable($this->custom_populate_columns[$column])) {
            // Run the function.
            call_user_func_array($this->custom_populate_columns[$column], array(
              $column,
              $post
            ));
  ...
  public function sortable($columns = array()) {
    ...
    $this->add_filter('manage_edit-' . $this->post_type_name . '_sortable_columns', array(
      &$this,
      'make_columns_sortable'
    ));
    // Run action that sorts columns on request.
    $this->add_action('load-edit.php', array(
      &$this,
      'load_edit'
    ));
   ...
   *
   * @param $bulk_messages array of bulk updated messages
   * @param $bulk_counts
   *
   * @return mixed
   */
